...
func getTables(tableNames string) []Table {
	...
	var tables []Table
	if tableNames == "" {
		db.Raw("SELECT TABLE_NAME as Name,TABLE_COMMENT as Comment FROM information_schema.TABLES WHERE table_schema='"+ config.DbName +"';").Find(&tables)
	}else{
		db.Raw("SELECT TABLE_NAME as Name,TABLE_COMMENT as Comment FROM information_schema.TABLES WHERE TABLE_NAME IN ("+tableNames+") AND table_schema='"+ config.DbName +"';").Find(&tables)
	}
	return tables
}
//获取所有字段信息
...
func generateModel(table Table, fields []Field)  {
	...
	filename := "./models/" +generator.CamelCase(table.Name)+".go"
	var f *os.File
	var err error
